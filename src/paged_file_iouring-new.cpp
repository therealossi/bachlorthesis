...
#define BlockSize (1024 * 1024)  //File size 
...
static uio::task<> read_iouring(char* returnChar, off_t offset, int file, int wsize = BlockSize){
    co_await service.read(file, returnChar, wsize, offset);
}   
...
static uio::task<> write_iouring(char* data, off_t offset, int file, int wsize = BlockSize){
    co_await service.write(file, data, wsize, offset);
}
...
bool paged_file::open_(const std::string& path, int file_type) {
    file_name_ = path;
    std::filesystem::path path_obj(path);
    uio::task finalTask;
    // check if path exists and is of a regular file
    if (! std::filesystem::exists(path_obj)) {
        file_ = open(path.c_str(), O_RDWR | O_CREAT , 0644);
        header_.ftype_ = file_type;
        header_.slots_.reset();
        memset(header_.payload_, 0, FHEADER_PAYLOAD_SIZE);
        finalTask = write_iouring((char *)&header_, 0, file_, sizeof(header_));
	    service.run(finalTask);
        ...
        // read & check header
        finalTask = read_iouring((char *) &header_, 0, file_, sizeof(header_));
	    service.run(finalTask);
    ...
    auto file_size = lseek(file_, 0, SEEK_END);
    npages_ = (file_size - sizeof(struct file_header)) / PF_PAGE_SIZE;
...
void paged_file::close_file() {
    ...
    auto finalTask = write_iouring((char *)&header_, 0, file_, sizeof(header_));
	service.run(finalTask);
    // file_.flush(); 
...
paged_file::page_id paged_file::allocate_page() {
    ...
    if (pid != UNKNOWN) {
        ...
        finalTask = write_iouring((char *) buf, pid * PF_PAGE_SIZE + sizeof(file_header), file_);
        pid += 1;
    }
    else {
        // append a page to the file
        //file_.seekp(0, file_.end);
        //pid = (lseek(file_,0,SEEK_END) - sizeof(file_header)) / PF_PAGE_SIZE + 1;
        write_iouring2((char *) buf, BlockSize, file_);
        finalTask = write_iouring((char *) buf, sizeof(file_header) + (npages_++*PF_PAGE_SIZE), file_);
        pid = npages_;
    }
